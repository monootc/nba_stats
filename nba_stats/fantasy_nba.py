...
class Player:
    def __init__(self, name):
        if name not in PLAYER_LU_TABLE:
            raise Exception(f'Do not have information on player {name}')
        self.name = name.title()
        if self.name not in PLAYER_LU_TABLE:
            raise Exception(f'Do not have record for {self.name}')
        self.player_id = PLAYER_LU_TABLE[name]
        self._game_raw_data = {}
    ...
    def yearly_raw_data(self):
        ...
        self._yearly_raw_data = BeautifulSoup(raw_data.text)
        return self._yearly_raw_data
    ...
    def get_game_raw_data(self, year):
        ...
        if raw_data.status_code != 200:
            raise Exception(f'Error getting data from {url=}')
        self._game_raw_data[year] = BeautifulSoup(raw_data.text)
        return self._game_raw_data[year]
    ...
    def _get_table(self, game=True, year=None):
        try:
            ...
            table = data.find_all('table')[index]
            return table
        except Exception as e:
            print(f'Error getting table: {game=} {index}, {e.message}, {e.args}')
